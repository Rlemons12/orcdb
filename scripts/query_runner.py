...
QUERY_DEFINITIONS: dict[str, QueryDefinition] = {
    ...
    "person_parts_history": QueryDefinition(
        script_name="run_person_parts_history.py",
        description="Parts charged to work orders touched by a named employee.",
        sql_file="person_parts_history.sql",
        output_dir="person_parts_history",
        file_prefix="person_parts_history",
        sheet_name="Person Parts",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
            ArgSpec("--name", "NAME", "Full or partial employee name.", required=True),
            ArgSpec("--days", "DAYS", "Rolling number of days.", arg_type=int, default=365, sql_kind="int"),
        ),
    ),
    ...
    "work_order_time_charged": QueryDefinition(
        script_name="run_work_order_time_charged.py",
        description="Actual time charged to a work order by department and resource.",
        sql_file="work_order_time_charged.sql",
        output_dir="work_order_time_charged",
        file_prefix="work_order_time_charged",
        sheet_name="WO Time Charged",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
            ArgSpec("--work-order", "WORK_ORDER", "Work order number.", required=True),
            ArgSpec("--operation-seq", "OPERATION_SEQ", "Operation sequence number.", arg_type=int, default=10, sql_kind="int"),
        ),
    ),
    "asset_maintenance_burden_summary": QueryDefinition(
        script_name="run_asset_maintenance_burden_summary.py",
        description="Asset maintenance burden summary.",
        sql_file="asset_maintenance_burden_summary.sql",
        output_dir="asset_maintenance_burden_summary",
        file_prefix="asset_maintenance_burden_summary",
        sheet_name="Asset Burden",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
    ...
    "configured_bom_for_asset": QueryDefinition(
        ...
        sql_file="configured_bom_for_asset.sql",
        output_dir="configured_bom_for_asset",
        file_prefix="configured_bom_for_asset",
        sheet_name="Configured BOM",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
            ArgSpec("--asset-number", "ASSET_NUMBER", "Asset number.", required=True),
    ...
    "configured_boms_for_department": QueryDefinition(
        script_name="run_configured_boms_for_department.py",
        description="Official configured BOMs for all assets in a department.",
        sql_file="configured_boms_for_department.sql",
        output_dir="configured_boms_for_department",
        file_prefix="configured_boms_for_department",
        sheet_name="Department BOMs",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
            ArgSpec("--department-code", "DEPARTMENT_CODE", "Department code.", required=True, upper=True),
            ArgSpec("--operation-seq", "OPERATION_SEQ", "Operation sequence number. Use 0 for all operations.", arg_type=int, default=10, sql_kind="int"),
            ArgSpec("--active-only", "ACTIVE_ONLY", "Use 1 for active components only, 0 for all components.", arg_type=int, default=1, sql_kind="int"),
        ),
    ),
    "configured_boms_for_class_code": QueryDefinition(
        script_name="run_configured_boms_for_class_code.py",
        description="Official configured BOMs for all assets in a work order class code.",
        sql_file="configured_boms_for_class_code.sql",
        output_dir="configured_boms_for_class_code",
        file_prefix="configured_boms_for_class_code",
        sheet_name="Class BOMs",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
            ArgSpec("--class-code", "CLASS_CODE", "Work order class code, such as MMLFL or MMCBF.", required=True, upper=True),
            ArgSpec("--active-only", "ACTIVE_ONLY", "Use 1 for active components only, 0 for all components.", arg_type=int, default=1, sql_kind="int"),
        ),
    ),
    ...
    "top_assets_by_time_charged": QueryDefinition(
        ...
        sql_file="top_assets_by_time_charged.sql",
        output_dir="top_assets_by_time_charged",
        file_prefix="top_assets_by_time_charged",
        sheet_name="Top Time Charged",
        args=(
            ArgSpec("--org-code", "ORG_CODE", "Organization code.", default="XAU", upper=True),
            ArgSpec("--months", "MONTHS", "Rolling number of months.", arg_type=int, default=12, sql_kind="int"),
            ArgSpec("--limit", "LIMIT", "Maximum number of rows.", arg_type=int, default=10, sql_kind="int"),
