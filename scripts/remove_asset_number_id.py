"""Remove the asset_number_id surrogate key from asset_number."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rebuild asset_number without the asset_number_id column."
    )
...
def make_backup(db_path: Path) -> Path:
    ...
    backup_path = db_path.with_name(
        f"{db_path.stem}_before_remove_asset_number_id_{stamp}{db_path.suffix}"
    )
...
def rebuild_asset_number(connection: sqlite3.Connection) -> None:
    required = {"model_id", "asset_number", "asset_description"}
    missing = required - columns_for(connection, "asset_number")
    if missing:
        raise SystemExit(f"asset_number is missing column(s): {sorted(missing)}")

    if "asset_number_id" not in columns_for(connection, "asset_number"):
        print("asset_number_id is already removed")
        return
